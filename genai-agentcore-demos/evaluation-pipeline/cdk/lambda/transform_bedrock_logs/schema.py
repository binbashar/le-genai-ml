...
def extract_structured_record(bedrock_log: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(system_data, list):
        system_blocks = [
            block.get("text", "") for block in system_data if block.get("text")
        ]
    ...
    agent_name = _extract_agent_name_from_identity(
        identity_arn
    ) or _extract_model_family(model_id)
    date_partition = timestamp[:10] if timestamp else ""  # YYYY-MM-DD
    hour_partition = (
        timestamp[11:13] if timestamp and len(timestamp) >= 13 else "00"
    )  # HH
