...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not output_s3_uri:
        ...
        output_s3_uri = f"s3://{bucket_name}/evaluation-results/{agent_name}/{timestamp}/"
    ...
    max_samples = min(question_count, int(os.environ.get('MAX_EVALUATION_SAMPLES', '10')))

    logger.info(f"Creating evaluation job: {job_name}")
    logger.info(f"Dataset: {dataset_s3_uri} ({question_count} questions, limiting to {max_samples})")
    ...
    try:
        ...
        response = bedrock_client.create_evaluation_job(
            ...
            jobTags=[
                ...
                {
                    'key': 'Pipeline',
                    'value': 'evaluation-pipeline'
                },
                {
                    'key': 'CreatedBy',
                    'value': 'create-evaluation-job-lambda'
                }
            ]
        )
        ...
        logger.error(f"Validation error creating evaluation job: {str(e)}", exc_info=True)
        ...
        raise RuntimeError(f"Service quota exceeded. Please check Bedrock limits: {str(e)}")
        ...
        raise RuntimeError(f"IAM permissions insufficient. Check evaluation role: {evaluation_role_arn}")

    except Exception as e:
        logger.error(f"Unexpected error creating evaluation job: {str(e)}", exc_info=True)
