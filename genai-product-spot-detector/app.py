...
from utils import (
    ...
    parse_json_response,
    analyze_planogram_individually,
    analyze_realogram_individually,
    compare_planogram_vs_realogram,
)
from database import (
    guardar_analisis,
    calcular_porcentaje_cumplimiento,
    obtener_analisis_recientes,
    obtener_detalle_analisis,
)

# Configure the Streamlit page
st.set_page_config(
    page_title="Análisis de Planograma vs Realograma", page_icon="📊", layout="wide"
)

# Title and description
st.title("📊 Análisis de Planograma vs Realograma")
st.markdown(
    "Compara el planograma ideal con la imagen real del góndola para identificar diferencias y cumplimiento."
)
...
with main_tab1:
    ...
    if st.button("🚀 Iniciar Análisis", type="primary"):
        if planogram_file is not None and realogram_file is not None:
            with st.spinner("Analizando imágenes..."):
                try:
                    ...
                    if json_response is None:
                        st.error(
                            "No se pudo completar el análisis. Verifica que la clave API de Anthropic esté configurada correctamente."
                        )
                    else:
                        ...
                        if parsed_data is None:
                            ...
                            planogram_name = (
                                planogram_file.name
                                if hasattr(planogram_file, "name")
                                else "planograma.jpg"
                            )
                            realogram_name = (
                                realogram_file.name
                                if hasattr(realogram_file, "name")
                                else "realograma.jpg"
                            )

                            analisis_id = guardar_analisis(
                                planogram_name, realogram_name, json_response
                            )
                            if analisis_id:
                        ...
                        with tab1:
                            ...
                            if "diferencias" in parsed_data:
                                for difference in parsed_data.get("diferencias", []):
                                    ...
                                    productos = difference.get("resultado", {}).get(
                                        "productos", []
                                    )
                            ...
                            if all_products:
                                ...
                                def highlight_rows(row):
                                    if not row["encontrado"]:
                                        return ["background-color: #ffcccc"] * len(
                                            row
                                        )  # Red for missing
                                    elif not row["posicion_correcta"]:
                                        return ["background-color: #ffd700"] * len(
                                            row
                                        )  # Gold for wrong position
                                    else:
                                        return ["background-color: #ccffcc"] * len(
                                            row
                                        )  # Green for correct
                                ...
                                with col1:
                                    ...
                                    st.metric(
                                        ...
                                        f"{found_products} ({found_products/len(df)*100:.1f}%)",
                                    )
                                ...
                                with col2:
                                    ...
                                    st.metric(
                                        "Productos Faltantes",
                                        f"{missing_products} ({missing_products/len(df)*100:.1f}%)",
                                    )
                                ...
                                with col3:
                                    ...
                                    st.metric(
                                        ...
                                        f"{correct_pos} ({correct_pos/len(df)*100:.1f}%)",
                                    )
                                ...
                                st.warning(
                                    "No se encontraron datos de productos en los resultados del análisis."
                                )
                            ...
                            if "analisis_planograma" in parsed_data:
                                ...
                                if isinstance(planogram_data, dict):
                                    ...
                                    if "productos_detectados" in planogram_data:
                                        ...
                                        for producto in planogram_data[
                                ...
                                st.info(
                                    "No hay análisis individual del planograma disponible."
                                )
                            ...
                            if "analisis_realograma" in parsed_data:
                                ...
                                if isinstance(realogram_data, dict):
                                    ...
                                    if "productos_observados" in realogram_data:
                                        ...
                                        for producto in realogram_data[
                                ...
                                st.info(
                                    "No hay análisis individual del realograma disponible."
                                )
            ...
            st.warning(
                "⚠️ Por favor, sube ambas imágenes (planograma y realograma) antes de iniciar el análisis."
            )
...
with main_tab2:
    ...
    num_pairs = st.number_input(
        "Número de pares de imágenes a analizar", min_value=1, max_value=10, value=2
    )
                ...
                try:
                    ...
                    if json_response:
                        ...
                        if parsed_data:
                            ...
                            # Save to database
                            guardar_analisis(
                                planogram.name, realogram.name, json_response
                            )
    ...
    if st.session_state.multiple_results:
        ...
        styled_summary = summary_df.style.applymap(
            highlight_compliance, subset=["Cumplimiento (%)"]
        )
        ...
        for result in st.session_state.multiple_results:
            with st.expander(
                f"Detalles Par {result['pair_number']} - {result['compliance_percentage']:.1f}% cumplimiento"
            ):
    ...
    if recent_analysis:
        for analysis in recent_analysis:
            with st.sidebar.expander(
                f"ID: {analysis['id']} - {analysis['fecha_analisis'].strftime('%d/%m/%Y')}"
            ):
                ...
                if analysis["respuesta_json"]:
                    try:
                        compliance = calcular_porcentaje_cumplimiento(
                            analysis["respuesta_json"]
                        )
