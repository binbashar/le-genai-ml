            ...
            try:
                ...
                if model_id.startswith("amazon.nova"):
                    ...
                    request_body = {
                        ...
                        "system": [{"text": system_prompt if system_prompt else """Eres un asistente de voz amigable en español. 
                    ...
                    request_body = {
                        ...
                        "max_tokens": 300,
                        "temperature": 0.7,
                        "system": system_prompt if system_prompt else """Eres un asistente de voz amigable en español. 
                        Responde de manera natural, clara y concisa. Mantén un tono conversacional y cálido.
                        Tus respuestas deben ser apropiadas para ser leídas en voz alta.""",
                        "messages": [
                            {
                ...
                last_error = e
                logger.warning(f"❌ Modelo {model_id} falló: {e.response['Error']['Code']}")
                continue
...
def process_speech_to_speech(audio_file, clients, system_prompt=""):
    ...
    steps = ["🎵 Subir audio", "🎤 Transcribir", "🧠 Generar respuesta", "🗣️ Sintetizar voz"]
    current_step = 0
    ...
    try:
        ...
        update_steps(2)
        response_text = generate_response_with_claude(transcript, clients['bedrock'], system_prompt)
...
def main():
    ...
    if uploaded_file is not None:
        if st.button(
            ...
            # Preparar prompt
            system_prompt = custom_instructions if custom_instructions.strip() else personalities[selected_personality]
        ...
        for msg in st.session_state.messages:
            if msg["role"] == "user":
                ...
                if 'audio_data' in msg and msg['audio_data'] is not None:
                    ...
                    st.info(f"📁 Audio original: {msg.get('audio_name', 'archivo de audio')}")
