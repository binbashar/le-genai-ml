...
class MultiSiteScraper:
    ...
    def __init__(self):
        self.site_configs = {
            'priceshoes': {
                ...
                'selectors': {
                    'login_button': ['button:has-text("Ingresar a mi cuenta")', 'a:has-text("Ingresar a mi cuenta")', '[class*="account"]'],
                    'email': ['input[type="text"]', 'input[type="email"]', '#email', '[name="email"]'],
                    'password': ['input[type="password"]', '#password', '[name="password"]'],
                    'submit': ['button:has-text("Siguiente")', 'button:has-text("Ingresar")', 'button[type="submit"]'],
                    'orders': ['.order-item', '.pedido-item', '.order', '.pedido', '[class*="order"]', '[class*="pedido"]'],
            ...
            'avon': {
                ...
                'selectors': {
                    'login_button': ['a:has-text("Iniciar sesión")', 'a:has-text("Mi cuenta")', '[href*="login"]'],
                    'email': ['input[name="username"]', 'input[id="username"]', 'input[type="text"]', '#j_username'],
                    'password': ['input[name="password"]', 'input[id="password"]', 'input[type="password"]', '#j_password'],
                    'submit': ['button[type="submit"]', 'input[type="submit"]', 'button:has-text("Ingresar")', '[value="Ingresar"]'],
                    'orders': ['.pedido', '.order-history', '[class*="pedido"]', 'table.orders', '.mis-pedidos'],
                    'account_menu': ['Mi Cuenta', 'Mis Pedidos', 'Cerrar Sesión', 'Mi Espacio']
                }
            }
        }
    ...
    def _run_scraper(self, instructions: Dict, url: str, username: str, password: str, site_config: Dict) -> Dict:
        ...
        log(f"🚀 Iniciando scraper para {site_config['name']} con {len(instructions['steps'])} pasos")
        ...
        with sync_playwright() as p:
            ...
            try:
                for i, step in enumerate(instructions['steps']):
                    action = step['action']
                    value = step['value']
                    log(f"📍 Paso {i+1}/{len(instructions['steps'])}: {action} - {value}")
                    
                    if action == 'go_to':
                        # Manejar cookies antes de navegar
                        self._handle_popups(page, logs, screenshots)
                        
                        target_url = site_config['base_url'] if value == 'url_base' else value
                        ...
                        login_success = self._perform_login(page, username, password, site_config, logs, screenshots)
    ...
    def _perform_login(self, page, username: str, password: str, site_config: Dict, logs: List, screenshots: Dict) -> bool:
        ...
        try:
            ...
            if site_config['login_url'] not in page.url:
                ...
                if not login_clicked:
                    ...
                    page.goto(site_config['login_url'], wait_until='networkidle', timeout=30000)
            ...
            if site_config['login_type'] == 'two_step':
                ...
                result = self._two_step_login(page, username, password, site_config, logs, screenshots)
            else:
                # Login en un solo paso (Avon)
                result = self._single_step_login(page, username, password, site_config, logs, screenshots)
    ...
    def _single_step_login(self, page, username: str, password: str, site_config: Dict, logs: List, screenshots: Dict) -> bool:
    ...
    def _two_step_login(self, page, username: str, password: str, site_config: Dict, logs: List, screenshots: Dict) -> bool:
    ...
    def _extract_data(self, page, target: str, site_config: Dict) -> Dict:
        ...
        try:
            if target in ['body', 'all']:
                ...
                extracted["resumen"] = content[:500] + "..." if len(content) > 500 else content
                ...
                try:
                    ...
                    texts = [elem.inner_text().strip() for elem in elements if elem.inner_text().strip()]
                    ...
                    extracted["contenido"] = content[:2000] + "..." if len(content) > 2000 else content
    ...
    def _extract_orders(self, page, site_config: Dict) -> List[Dict]:
        ...
        if not orders:
            ...
            for table in tables:
                ...
                if any(word in table_text for word in ['pedido', 'order', 'compra', 'fecha', 'total', 'campaña', 'folio']):
    ...
    def _parse_avon_order(self, element) -> Optional[Dict]:
        ...
        try:
            ...
            if order:
                order['texto_completo'] = text[:300] + "..." if len(text) > 300 else text
                order['sitio'] = 'Avon'
                return order
    ...
    def _parse_order_element(self, element) -> Optional[Dict]:
        ...
        try:
            ...
            # Solo retornar si encontramos al menos algún dato
            if order:
                order['texto_completo'] = text[:200] + "..." if len(text) > 200 else text
                return order
    ...
    def _parse_order_table(self, table) -> List[Dict]:
        ...
        try:
            ...
            for i, row in enumerate(rows):
                ...
                if i == 0 or any('th' in cell.evaluate('el => el.tagName') for cell in cells):
                    headers = cell_texts
                    ...
                    if len(cell_texts) == len(headers):
                        ...
                        for header, value in zip(headers, cell_texts):
                            ...
                            if any(word in header_lower for word in ['pedido', 'order', 'número', 'number', 'id']):
                                order['numero_pedido'] = value
                            elif any(word in header_lower for word in ['fecha', 'date']):
                                order['fecha'] = value
                            elif any(word in header_lower for word in ['total', 'monto', 'amount']):
                                order['total'] = value
                            elif any(word in header_lower for word in ['estado', 'status']):
                                order['estado'] = value
                            else:
                                order[header] = value
    ...
    def _extract_orders_from_text(self, text: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if re.search(r'(?:Pedido|Order|#)\s*[:＃#]?\s*\d+', line, re.IGNORECASE):
                ...
                # Extraer número
                match = re.search(r'(?:Pedido|Order|#)\s*[:＃#]?\s*(\d+)', line, re.IGNORECASE)
                if match:
    ...
    def _extract_tables(self, page) -> List[Dict]:
        ...
        try:
            ...
            for idx, table in enumerate(tables):
                ...
                # Extraer headers
                headers = table.query_selector_all('thead th, tr:first-child th, tr:first-child td')
                ...
                # Crear resumen
                if table_info["filas"]:
                    table_info["resumen"] = f"Tabla con {len(table_info['headers'])} columnas y {len(table_info['filas'])} filas"
    ...
    def _extract_account_info(self, page, site_config: Dict) -> Dict:
        ...
        try:
            ...
            info_patterns = {
                ...
                'id_cliente': [r'(?:ID Cliente|Customer ID|Número de socio)\s*[:：]?\s*(\w+)'],
