...
def lambda_handler(event, context):
    ...
    if function == "customerId":
        ...
        purchaseId = unpacked.get("purchaseId", None)
        supportSummary = unpacked.get("supportSummary", None)

        if all(
            value is not None and value != ""
            for value in (custId, purchaseId, supportSummary)
        ):
    ...
    elif function == "purchaseSearch":
        custId = unpacked.get("custId", None)
        purchaseDate = unpacked.get("purchaseDate", None)
        productDescription = unpacked.get("productDescription", None)

        if all(
            value is not None and value != ""
            for value in (custId, purchaseDate, productDescription)
        ):
            purchase_id = random.randint(1000, 9999)
            responseBody["TEXT"]["body"] = f"{{'purchaseId':{purchase_id}}}"
        else:
            responseBody["TEXT"]["body"] = "{'error':'Purchase not found.'}"
