        ...
        if "chunk" in event:
            ...
            if "trace" in trace:
                ...
                if "orchestrationTrace" in trace_details:
                    ...
                    if "invocationInput" in orch_trace:
                        ...
                        if "actionGroupInvocationInput" in inv_input:
                            ...
                            print(
                                f"  Action Group: {agi.get('actionGroupName', 'N/A')}"
                            )
                    ...
                    if "observation" in orch_trace:
                        ...
                        if "actionGroupInvocationOutput" in obs:
                            print(
                                f"  Action Group Output: {obs['actionGroupInvocationOutput'].get('text', 'N/A')}"
                            )
                        if "knowledgeBaseLookupOutput" in obs:
                            print("  Knowledge Base Lookup:")
                            for ref in obs["knowledgeBaseLookupOutput"].get(
                                "retrievedReferences", []
                            ):
                                print(
                                    f"    - {ref['content'].get('text', 'N/A')[:50]}..."
                                )
                        if "codeInterpreterInvocationOutput" in obs:
                            cio = obs["codeInterpreterInvocationOutput"]
                            print("  Code Interpreter Output:")
                            print(
                                f"    Execution Output: {cio.get('executionOutput', 'N/A')[:50]}..."
                            )
                            print(
                                f"    Execution Error: {cio.get('executionError', 'N/A')}"
                            )
                            print(
                                f"    Execution Timeout: {cio.get('executionTimeout', 'N/A')}"
                            )
                ...
                if "guardrailTrace" in trace_details:
                    ...
                    for assessment in guard_trace.get(
                        "inputAssessments", []
                    ) + guard_trace.get("outputAssessments", []):
                        if "contentPolicy" in assessment:
                            for filter in assessment["contentPolicy"].get(
                                "filters", []
                            ):
                                print(
                        ...
                        if "sensitiveInformationPolicy" in assessment:
                            for pii in assessment["sensitiveInformationPolicy"].get(
                                ...
                                print(
                                    f"  PII Detected: {pii['type']} (Action: {pii['action']})"
                                )
...
def wait_for_action_group_status(
    agentId: str, actionGroupId: str, targetStatus: str = "ENABLED"
) -> str:
    """
