...
def get_instruction(lesson):
    instruction_files = {
        "lesson2": "shared/instructions/instruction_stage_1.txt",
        "lesson5": "shared/instructions/instruction_stage_3.txt",
    }
    default_file = "shared/instructions/instruction_stage_2.txt"
...
def create_lambda_function(lambda_client, function_name, role_arn, lesson):
    ...
    lambda_file = (
        "shared/functions/lambda_stage_1.py"
        if lesson == "lesson_2"
        else "shared/functions/lambda_stage_2.py"
    )
...
def main():
    ...
    alias_response = bedrock_agent.create_agent_alias(
        agentAliasName="MyAgentAlias", agentId=agent_id
    )
    agent_alias_id = alias_response["agentAlias"]["agentAliasId"]
    # print(f"agentId: {agent_id}, agentAliasId: {agent_alias_id}")

    wait_for_agent_alias_status(
        agentId=agent_id, agentAliasId=agent_alias_id, targetStatus="PREPARED"
    )
    ...
    function_arn = create_lambda_function(
        lambda_client, function_name, lambda_role_arn, lesson
    )
