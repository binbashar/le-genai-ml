...
def calculate_metrics(result: Dict) -> Dict[str, Any]:
    ...
    for nivel in result["diferencias"]:
        if "resultado" in nivel and "productos" in nivel["resultado"]:
            for producto in nivel["resultado"]["productos"]:
                ...
                # Check frente issues
                if frentes_expected != frentes_found and producto.get(
                    "encontrado", False
                ):
    ...
    if metrics["total"] > 0:
        ...
        metrics["position_accuracy"] = (
            metrics["correct_position"] / metrics["found"]
            if metrics["found"] > 0
            else 0
        )
        ...
        if metrics["total_frentes_expected"] > 0:
            frentes_score = (
                min(
                    1.0,
                    metrics["total_frentes_found"] / metrics["total_frentes_expected"],
                )
                * frentes_weight
            )
            metrics["frentes_accuracy"] = (
                metrics["total_frentes_found"] / metrics["total_frentes_expected"]
            )
        else:
        ...
        if "resultado" in nivel and "productos" in nivel["resultado"]:
            ...
            for producto in productos:
                ...
                if frentes_encontrados is None:
                    frentes_encontrados = producto.get(
                        "cantidad_frentes_encontrados", 0
                    )

                if frentes_esperados != frentes_encontrados and producto.get(
                    "encontrado", False
                ):
...
def generate_compliance_report(result: Dict, metrics: Dict) -> str:
    ...
    report.append(
        f"• Productos Encontrados: {metrics['found']}/{metrics['total']} ({metrics['recall']:.1%})"
    )
    report.append(
        f"• Posiciones Correctas: {metrics['correct_position']}/{metrics['total']} ({metrics['precision']:.1%})"
    )
    ...
    if metrics["products_with_frente_issues"] > 0:
        report.append(
            f"⚠️ PRODUCTOS CON DIFERENCIAS EN FRENTES: {metrics['products_with_frente_issues']}"
        )
    report.append("")
        ...
        for nivel in result["diferencias"]:
            ...
            if "resultado" in nivel and "productos" in nivel["resultado"]:
                for producto in nivel["resultado"]["productos"]:
                    ...
                    frentes = (
                        f"{frentes_encontrados}/{producto.get('frentes_esperados', 0)}"
                    )

                    report.append(f"  • {nombre}")
                    report.append(
                        f"    Encontrado: {encontrado} | Posición: {posicion} | Frentes: {frentes}"
                    )
